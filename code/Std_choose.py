...
def select_image(img_list):
    ...
    while selected_index is None:
        user_input = input("Please select an image sequence number for the graphic (enter sequence number) or enter a new standard deviation (enter s): PS: Please note that if your platform is Windows, the image will automatically pop up. If you are using a pure character platform similar to Shell, please enter the images in the Generatre_UMAP folder. Thank you for your understanding")
        if user_input == "s":
