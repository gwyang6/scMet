...
def loop_choose_std(best_model, cell_type_proportions_df, common_genes_metabolism, data_metabolism, anno,
    ...
    for z_std in z_std_list:
        ...
        sc.tl.umap(adata)
        save_path = os.path.join(os.getcwd(), "output/Simulation_UMAP_Metabolism_std_" + str(z_std) + ".png")
...
def select_image(img_list):
    ...
    while selected_index is None:
        print('!!! Please note that if your platform is Windows, the image will automatically pop up. If you are using a pure character platform similar to Shell, please enter the images in the scMet/code/output folder. Thank you for your understanding')
        user_input = input("Please select an image sequence number for the graphic (enter sequence number) or enter a new standard deviation (enter s):" )
        if user_input == "s":
