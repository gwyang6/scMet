...
def preprocess_and_plot_umap_metabolism(bulk_data, data, anno, metabolism_genes, save_path=None):
    ...
    if save_path is None:
        save_path = os.path.join(os.getcwd(), "output/Orig_UMAP_Metabolism.png")
    else:
...
class CVAE(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        # Encoder output z_Mean and z_Log_var
        h = self.encoder(x)
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_data_batches):
            ...
            with tf.GradientTape() as tape:
                ...
                recon_x, z_mean, z_log_var = cvae((x, c))
                mse_loss = mse_loss_fn(x, recon_x)
                def kl_divergence(z_mean, z_log_var):
                    z_log_var = tf.clip_by_value(z_log_var, -1e6, 0)
                    kl_loss = -0.5 * tf.reduce_mean(1 + z_log_var - tf.square(z_mean) - tf.exp(z_log_var))
                    kl_loss = tf.clip_by_value(kl_loss, -1e10, 1e10)  # 对 KL 散度结果进行裁剪
                    return kl_loss
        ...
        for val_batch in val_data_batches:
            ...
            val_recon_x, val_z_mean, val_z_log_var = cvae((val_x, val_c))
