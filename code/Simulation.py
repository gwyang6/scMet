...
def simulate_bulk_data(cell_type_proportions_df, df, bulk_metabolism_ratio,common_genes_metabolism, num_simulations=20, num_cells=5000 ):
    ...
    for sample in range(cell_type_proportions_df.shape[0]):
        ...
        save_path = os.path.join(os.getcwd(),f"{output/cell_type_proportions_df.index[sample]}_corr.png")
...
def generate_umap_plots(selected_gene_expression_df_total, selected_celltype, selected_samplename, save_path=None):
    ...
    if save_path is None:
        save_path_celltype = os.path.join(os.getcwd(), "output/Bulk_Simulation_UMAP_celltype.png")
        save_path_sample = os.path.join(os.getcwd(), "output/Bulk_Simulation_UMAP_sample.png")
    else:
        save_path_celltype = os.path.join(save_path, "output/Bulk_Simulation_UMAP_celltype.png")
        save_path_sample = os.path.join(save_path, "output/Bulk_Simulation_UMAP_sample.png")

    print(f"Total generated cells: {selected_gene_expression_df_total.shape[0]}")
    print(f"UMAP plots will be saved to: {save_path}")

    sc.pl.umap(adata, color=['celltype'], legend_loc='on data', title='', frameon=True, show=False)
    plt.savefig(save_path_celltype)
    plt.close()
    sc.pl.umap(adata, color=['celltype'], legend_loc='on data', title='', frameon=True)
    sc.pl.umap(adata, color=['sample'], legend_loc='on data', title='', frameon=True, show=False)
    plt.savefig(save_path_sample)
    plt.close()
    sc.pl.umap(adata, color=['sample'], legend_loc='on data', title='', frameon=True)
    return adata