...
def simulate_bulk_data_pre(data, bulk_data, num_simulations=10, num_cells=600):
    ...
    for i in range(num_simulations):
        selected_cells = data.sample(n=num_cells, axis=1,replace=True)
...
def generate_synthetic_data(best_model, cell_type_proportions_df, common_genes_metabolism, data_metabolism, anno,
    ...
    def truncate_data(data, lower_bound, upper_bound):
        # 将数据截断到指定的范围内
    ...
    lower_bound = -1
    upper_bound = 1
    z = truncate_data(z, lower_bound, upper_bound)
    ...
    sc.pp.log1p(adata)
    sc.pp.highly_variable_genes(adata, n_top_genes=350)
    sc.pp.scale(adata)
